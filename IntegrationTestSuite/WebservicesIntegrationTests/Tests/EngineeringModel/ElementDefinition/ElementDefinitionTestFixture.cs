...
namespace WebservicesIntegrationTests
{
    ...
    public class ElementDefinitionTestFixture : WebClientTestFixtureBase
    {
        ...
        public static void VerifyProperties(JArray jArray)
        {
            ...
            var expectedParameters = new[] { "6c5aff74-f983-4aa8-a9d6-293b3429307c" };
            ...
            elementDefinition = jArray.SingleOrDefault(
                x => (string)x[PropertyNames.Iid] == "f73860b2-12f0-43e4-b8b2-c81862c0a160");
            ...
            // assert that the properties are what is expected
            Assert.AreEqual("f73860b2-12f0-43e4-b8b2-c81862c0a160", (string)elementDefinition[PropertyNames.Iid]);
            Assert.AreEqual(1, (int)elementDefinition[PropertyNames.RevisionNumber]);
            Assert.AreEqual("ElementDefinition", (string)elementDefinition[PropertyNames.ClassKind]);
            ...
            expectedContainedElements = new string[] { };
            containedElementsArray = (JArray)elementDefinition[PropertyNames.ContainedElement];
            containedElements = containedElementsArray.Select(x => (string)x).ToList();
            CollectionAssert.AreEquivalent(expectedContainedElements, containedElements);

            expectedParameters = new string[] { };
            parametersArray = (JArray)elementDefinition[PropertyNames.Parameter];
            parameters = parametersArray.Select(x => (string)x).ToList();
            CollectionAssert.AreEquivalent(expectedParameters, parameters);

            expectedParameterGroups = new string[] { };
            parameterGroupsArray = (JArray)elementDefinition[PropertyNames.ParameterGroup];
            parameterGroups = parameterGroupsArray.Select(x => (string)x).ToList();
            CollectionAssert.AreEquivalent(expectedParameterGroups, parameterGroups);

            expectedreferencedElements = new string[] { };
            referencedElementsArray = (JArray)elementDefinition[PropertyNames.ReferencedElement];
            referencedElements = referencedElementsArray.Select(x => (string)x).ToList();
            CollectionAssert.AreEquivalent(expectedreferencedElements, referencedElements);

            Assert.AreEqual("0e92edde-fdff-41db-9b1d-f2e484f12535", (string)elementDefinition[PropertyNames.Owner]);

            expectedCategories = new string[] { };
            categoriesArray = (JArray)elementDefinition[PropertyNames.Category];
            categories = categoriesArray.Select(x => (string)x).ToList();
            CollectionAssert.AreEquivalent(expectedCategories, categories);

            expectedAliases = new string[] { };
            aliasesArray = (JArray)elementDefinition[PropertyNames.Alias];
            aliases = aliasesArray.Select(x => (string)x).ToList();
            CollectionAssert.AreEquivalent(expectedAliases, aliases);

            expectedDefinitions = new string[] { };
            definitionsArray = (JArray)elementDefinition[PropertyNames.Definition];
            definitions = definitionsArray.Select(x => (string)x).ToList();
            CollectionAssert.AreEquivalent(expectedDefinitions, definitions);

            expectedHyperlinks = new string[] { };
            hyperlinksArray = (JArray)elementDefinition[PropertyNames.HyperLink];
