    ...
    public class DecompositionRuleTestFixture : WebClientTestFixtureBaseWithDatabaseRestore
    {
        ...
        public void VerifyThatADecompositionRuleCanBeCreatedWithWebApi()
        {
            ...
            var expectedRules = new string[]
            {
                ...
                "11a67246-c08d-43e2-9ef5-b7dd8f3ca687"
            };
            ...
            Assert.That(containedCategories, Is.EquivalentTo(expectedContainedCategories));

            var expectedRuleAliases = new string[] { };
            var ruleAliasesArray = (JArray)decompositionRule["alias"];
            IList<string> ruleAliases = ruleAliasesArray.Select(x => (string)x).ToList();
            Assert.That(ruleAliases, Is.EquivalentTo(expectedRuleAliases));

            var expectedRuleDefinitions = new string[] { };
            var ruleDefinitionsArray = (JArray)decompositionRule["definition"];
            IList<string> ruleDefinitions = ruleDefinitionsArray.Select(x => (string)x).ToList();
            Assert.That(ruleDefinitions, Is.EquivalentTo(expectedRuleDefinitions));

            var expectedRuleHyperlinks = new string[] { };
            var ruleHyperlinksArray = (JArray)decompositionRule["hyperLink"];
            IList<string> hyperLinks = ruleHyperlinksArray.Select(x => (string)x).ToList();
            Assert.That(hyperLinks, Is.EquivalentTo(expectedRuleHyperlinks));
        }
