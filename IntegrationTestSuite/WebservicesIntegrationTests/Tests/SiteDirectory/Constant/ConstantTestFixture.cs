// --------------------------------------------------------------------------------------------------------------------
// <copyright file="ConstantTestFixture.cs" company="RHEA System S.A.">
//
...
namespace WebservicesIntegrationTests
{
    ...
    [TestFixture]
    public class ConstantTestFixture : WebClientTestFixtureBaseWithDatabaseRestore
    {
        [Test]
        [Category("GET")]
        ...
        public void VerifyThatAConstantCanBeCreatedWithWebApi()
        {
            ...
            var expectedConstants = new string[]
            {
                ...
                "8ef99572-3d39-44a4-9212-159bc82f2320"
            };
            ...
            var expectedConstantAliases = new string[] { };
            var aliasesConstantArray = (JArray)constant[PropertyNames.Alias];
            IList<string> constantAliases = aliasesConstantArray.Select(x => (string)x).ToList();
            CollectionAssert.AreEquivalent(expectedConstantAliases, constantAliases);

            var expectedConstantDefinitions = new string[] { };
            var definitionsConstantArray = (JArray)constant[PropertyNames.Definition];
            IList<string> constantDefinitions = definitionsConstantArray.Select(x => (string)x).ToList();
            CollectionAssert.AreEquivalent(expectedConstantDefinitions, constantDefinitions);

            var expectedConstantHyperlinks = new string[] { };
            var hyperlinksConstantArray = (JArray)constant[PropertyNames.HyperLink];
            IList<string> hyperLinks = hyperlinksConstantArray.Select(x => (string)x).ToList();
            CollectionAssert.AreEquivalent(expectedConstantHyperlinks, hyperLinks);
        }
